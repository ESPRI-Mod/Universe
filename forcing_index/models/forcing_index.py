...
linkml_meta = LinkMLMeta({'default_curi_maps': ['semweb_context'],
     'default_prefix': 'forcing',
     'default_range': 'string',
     'id': 'http://127.0.0.1:8000/uri/ForcingSchema',
     'license': 'https://creativecommons.org/publicdomain/zero/1.0/',
     'name': 'forcing_schema',
     'prefixes': {'esvocab': {'prefix_prefix': 'esvocab',
                              'prefix_reference': 'http://127.0.0.1:8000/uri/'},
                  'forcing': {'prefix_prefix': 'forcing',
                              'prefix_reference': 'http://127.0.0.1:8000/uri/forcing/'},
     ...
     'source_file': 'schemas/forcing_index.yaml',
     'title': 'forcing schema',
     'types': {'string': {'base': 'str',
                          'description': 'A character string',
                          'exact_mappings': ['schema:Text'],
                          'from_schema': 'http://127.0.0.1:8000/uri/ForcingSchema',
                          'name': 'string',
...
class ForcingIndex(ConfiguredBaseModel):
    ...
    """
    linkml_meta: ClassVar[LinkMLMeta] = LinkMLMeta({'class_uri': 'esvocab:forcing',
         'from_schema': 'http://127.0.0.1:8000/uri/ForcingSchema'})

    id: str = Field(..., json_schema_extra = { "linkml_meta": {'alias': 'id', 'domain_of': ['forcing_index']} })
    validation_method: str = Field("regex", json_schema_extra = { "linkml_meta": {'alias': 'validation_method',
         'domain_of': ['forcing_index'],
         'ifabsent': 'string(regex)'} })
    regex: str = Field(..., json_schema_extra = { "linkml_meta": {'alias': 'regex', 'domain_of': ['forcing_index']} })
    description: Optional[str] = Field(None, json_schema_extra = { "linkml_meta": {'alias': 'description', 'domain_of': ['forcing_index']} })
